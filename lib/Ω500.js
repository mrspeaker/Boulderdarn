	...
	var Entity = Ω.Class.extend({
		...
		*/
		move: function (x, y, map) {
			...
			// if overlapping edges, move back a little
			if (x < 0 && (!xBlocks[0].walkable || !xBlocks[1].walkable)) {
				xo = map.getBlockEdge(xv + map.sheet.w) - this.x;
				hitX = true;
			}
			if (x > 0 && (!xBlocks[2].walkable || !xBlocks[3].walkable)) {
			...
			if (yBlocks[0].walkable && yBlocks[1].walkable) {
				this.falling = true;
			} else {
				this.falling = false;
			}
...
(function (Ω) {
	...
	*/
	function initCanvas(canvasSelector, w, h) {
		...
		var selCanvas = typeof canvasSelector === "string" ? document.getElementById(canvasSelector) : canvasSelector,
			newCanvas,
			ctx;

		if (selCanvas === null) {
			console.error("Canvas DOM container not found:", canvasSelector);
			selCanvas = document.body;
		}
